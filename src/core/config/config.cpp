...
std::string quoted_value(const std::string& line) {
  ...
  return first == std::string::npos || last <= first ? std::string{}
...
bool is_legacy_default(const std::vector<ShortcutConfig>& entries) {
  ...
  const auto expected = std::array{
      std::pair{"cmd+c", "Clicked"}, std::pair{"cmd+v", "Pasted"}, std::pair{"cmd+x", "Cut"},
      std::pair{"cmd+s", "Saved"},   std::pair{"cmd+z", "Undo"},   std::pair{"cmd+shift+z", "Redo"},
  };
  for (std::size_t index = 0; index < expected.size(); ++index) {
    const auto parsed = parse_shortcut(expected[index].first);
    const bool legacy_click_label =
        index == 0 && (entries[index].label == "Copied" || entries[index].label == "Clicked");
    if (!parsed || entries[index].shortcut != *parsed ||
        (!legacy_click_label && entries[index].label != expected[index].second) ||
