...
int run() {
  @autoreleasepool {
    ...
    const auto config_path = std::filesystem::path(std::string([[NSHomeDirectory()
        stringByAppendingPathComponent:@"Library/Application Support/Oatmeal/shortcuts.conf"]
        UTF8String]));
    ...
    oatmeal::Listener listener(settings.shortcuts, [&](const oatmeal::ShortcutConfig &config) {
      const auto count = aggregator.record(config.shortcut, std::chrono::steady_clock::now());
      overlay->show(oatmeal::format_shortcut(config.shortcut), config.label, count);
    });
    listener.start();
