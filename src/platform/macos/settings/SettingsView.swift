...
public typealias OatmealSettingsCallback = @convention(c) (Int32, Int32, UnsafePointer<CChar>?) -> Void
...
private final class SettingsViewModel: ObservableObject {
    ...
    init(shortcuts: String, theme: Int32, position: Int32, duration: Double, callback: @escaping OatmealSettingsCallback) {
        self.shortcuts = shortcuts.split(separator: "\n").enumerated().compactMap { index, line in
            ...
            return ShortcutRow(id: index, configKey: String(fields[0]), label: String(fields[1]), enabled: fields[2] == "1")
        }
    ...
    func startRecording() {
        ...
        monitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            self.shortcuts.append(ShortcutRow(id: self.shortcuts.count, configKey: configKey, label: label, enabled: true))
            ...
            List {
                Section("Watched shortcuts") {
                    ForEach(Array(model.shortcuts.enumerated()), id: \.element.id) { index, shortcut in
                        HStack(spacing: 12) {
                            ...
                            TextField("Label", text: Binding(get: { shortcut.label }, set: { model.updateLabel(index, $0) }))
                                .textFieldStyle(.roundedBorder)
                            Button { model.remove(index) } label: { Image(systemName: "trash") }
                ...
                Section("Overlay") {
                    Picker("Theme", selection: Binding(get: { model.theme }, set: { model.setTheme($0) })) {
                        Text("Dark").tag(0); Text("Light").tag(1); Text("Graphite").tag(2)
                    }
                    Picker("Position", selection: Binding(get: { model.position }, set: { model.setPosition($0) })) {
                        Text("Top center").tag(0); Text("Bottom center").tag(1); Text("Top left").tag(2); Text("Top right").tag(3)
                    }
                    HStack {
                        Text("Display duration")
                        Slider(value: Binding(get: { model.duration }, set: { model.setDuration($0) }), in: 0.25...4)
