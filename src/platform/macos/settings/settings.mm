...
extern "C" void oatmeal_swift_show_settings(const char *shortcuts, int32_t theme, int32_t position,
...
std::string shortcut_payload() {
  ...
  for (const auto &entry : active_model->shortcuts) {
    output << config_key(entry.shortcut) << "|" << entry.label << "|" << (entry.enabled ? "1" : "0")
           << "\n";
  }
  return output.str();
}
...
bool sync_launch_agent(const SettingsModel &model) {
  ...
  const auto plist = std::string("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n") +
                     ...
                     "<key>ProgramArguments</key><array><string>/usr/bin/open</string><string>-a</"
                     "string><string>" +
...
void swift_callback(int32_t action, int32_t index, const char *value) {
  ...
  switch (action) {
    case 1:
      if (item < active_model->shortcuts.size())
        active_model->shortcuts[item].enabled = value != nullptr && std::string(value) == "1";
      break;
    case 2:
      if (item < active_model->shortcuts.size())
        active_model->shortcuts[item].label = value != nullptr ? value : "";
      break;
    case 3:
      if (item < active_model->shortcuts.size())
        active_model->shortcuts.erase(active_model->shortcuts.begin() + item);
      break;
    case 4:
    ...
    case 6:
      if (value != nullptr)
        active_model->overlay.duration = std::max(0.25, std::stod(value));
      break;
    case 7:
      if (value != nullptr) {
        const auto parsed = parse_shortcut(value);
        if (parsed)
          active_model->shortcuts.push_back({*parsed, parsed->key, true});
      }
      break;
    case 8:
      active_model->save();
...
SettingsModel SettingsModel::load(const std::filesystem::path &config_path) {
  ...
  model.launch_agent_path = std::filesystem::path(std::string([[NSHomeDirectory()
      stringByAppendingPathComponent:@"Library/LaunchAgents/com.oatmeal.app.plist"] UTF8String]));
  model.shortcuts = load_config(config_path);

  const auto launch_on_login_str = value_for(model.preferences_path, "launch_on_login");
  model.launch_on_login =
      (launch_on_login_str == "1") || std::filesystem::exists(model.launch_agent_path);

  const auto theme = value_for(model.preferences_path, "theme");
  if (theme == "light")
    model.overlay.theme = OverlayTheme::Light;
  else if (theme == "graphite")
    model.overlay.theme = OverlayTheme::Graphite;

  const auto position = value_for(model.preferences_path, "position");
  if (position == "bottom-center")
    model.overlay.position = OverlayPosition::BottomCenter;
  else if (position == "top-left")
    model.overlay.position = OverlayPosition::TopLeft;
  else if (position == "top-right")
    model.overlay.position = OverlayPosition::TopRight;

  const auto duration = value_for(model.preferences_path, "duration");
  if (!duration.empty())
    model.overlay.duration = std::max(0.25, std::stod(duration));
  return model;
}

bool SettingsModel::save() const {
  if (!write_config(config_path, shortcuts))
    return false;
  std::ofstream output(preferences_path);
  if (!output)
    return false;
  const char *theme = overlay.theme == OverlayTheme::Light      ? "light"
                      : overlay.theme == OverlayTheme::Graphite ? "graphite"
                                                                : "dark";
  const char *position = overlay.position == OverlayPosition::BottomCenter ? "bottom-center"
                         : overlay.position == OverlayPosition::TopLeft    ? "top-left"
                         : overlay.position == OverlayPosition::TopRight   ? "top-right"
                                                                           : "top-center";
...
void show_settings_window() {
  ...
  oatmeal_swift_show_settings(payload.c_str(), static_cast<int32_t>(active_model->overlay.theme),
                              static_cast<int32_t>(active_model->overlay.position),
                              active_model->overlay.duration, active_model->launch_on_login ? 1 : 0,
                              &swift_callback);
}
